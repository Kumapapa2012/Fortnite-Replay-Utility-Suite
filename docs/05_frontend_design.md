...
> **実装ノート (2026-05):** 当初設計からルート構成が変更されている。以下が現在の実装。

| Path | コンポーネント | 用途 |
|---|---|---|
| `/` | `<Dashboard />` | 現在のログ監視ステータス・バナー表示 |
| `/replays` | `<ReplayList />` | `.replay` ファイル一覧 |
| `/replays/:id` | `<ReplayDetail />` | リプレイ詳細（パース結果・HTML レポート） |
| `/replays/:id/map` | `<ReplayMap />` | 移動軌跡マップ表示 |
| `/matches` | `<MatchLibrary />` | Match Library（replay+video ペア一覧） |
| `/matches/:id` | `<MatchDetail />` | マッチ詳細（サイドカー状態・トリム・集計） |
| `/videos` | `<Videos />` | OBS 録画動画一覧・トリミング操作 |
| `/logs` | `<LogStream />` | 監視状態（ログパス・OBS・マッチ数）+ イベントストリーム表示 |
| `/settings` | `<Settings />` | グローバル設定（プレイヤー名・ディレクトリ等） |
| `*` | Navigate to `/` | 未マッチはルートにリダイレクト |
...
- `<Layout />`: サイドバー + `<Outlet />`
- 初回セットアップ画面（`/setup`）は未実装。設定は `/settings` ページで手動入力する。
...
|---|---|---|
| ダッシュボード | `/` | ログ監視バナー + 概要 |
| リプレイ | `/replays` | replay_parser 連携 |
| マッチ | `/matches` | Match Library（メイン機能） |
| 動画 | `/videos` | OBS 録画・トリミング |
| ログ | `/logs` | 監視状態確認 + イベント履歴 |
| 設定 | `/settings` | グローバル設定 |
...
> **実装ノート (2026-05):** log_monitor_api がサービス起動と同時に監視を自動開始するため、開始・停止ボタンは廃止した。

```
┌─ 監視状態 ───────────────────────────────────────┐
│ ログファイル  C:\Users\xxx\...\FortniteGame.log   │
│ OBS          接続済み                             │
│ マッチ数      3                                   │
└───────────────────────────────────────────────────┘
...
┌─ イベントログ  42 件 ─────────────────────────────┐
│ 時刻     │   │ フェーズ      │ ラベル              │
│ 19:35:42 │ ⚔️ │ ingame       │ 試合開始（降下可能） │
│ 19:35:01 │ ⏳ │ warmup       │ ウォームアップ開始   │
│ ...                                               │
└───────────────────────────────────────────────────┘
```

- **監視状態セクション**: ログパス・OBS 接続状態（Watchdog の復帰状況を反映）・マッチ数を表示。編集操作なし
- **イベントログセクション**: 直近イベントを新しい順に表示。`type: event`（ゲームイベント）と `type: system`（自動処理・OBS Watchdog 通知）を両方表示
- 空状態メッセージ: 「Fortnite を起動すると表示されます」
- データソース: SSE コンテキスト（`useLogMonitor()`）の `status` と `events` を直接参照
...
### 12.4 invalidate / refetch のタイミング
...
- もしくは Zustand に格納（§13.4 参照）

---

## 13. 状態管理方針

### 13.1 TanStack Query が担う範囲（サーバ状態）
...
## 15. 型定義（TypeScript）の管理方針

### 15.1 API レスポンス型の置き場所
...
### 15.2 snake_case ↔ camelCase 変換方針（Q4 確定: (b)）
...
│   ├── App.tsx                        # Routes 定義
│   ├── pages/
│   │   ├── Dashboard.tsx              # / — ログ監視バナー + 概要
│   │   ├── MatchLibrary.tsx           # /matches — Match Library 一覧
│   │   ├── MatchDetail.tsx            # /matches/:id — マッチ詳細・集計・トリム
│   │   ├── LogStream.tsx              # /logs — ログ監視コントロール + イベント
│   │   ├── ReplayList.tsx             # /replays — リプレイ一覧
│   │   ├── ReplayDetail.tsx           # /replays/:id — リプレイ詳細
│   │   ├── ReplayMap.tsx              # /replays/:id/map — マップ表示
│   │   ├── Videos.tsx                 # /videos — 録画動画・トリミング
│   │   ├── Settings.tsx               # /settings — グローバル設定
│   │   └── Placeholder.tsx            # 未実装ページ汎用
│   ├── components/
│   │   ├── Layout.tsx                 # サイドバー + Outlet
│   │   ├── PageHeader.tsx             # ページ共通ヘッダー
│   │   ├── LogMonitorBanner.tsx       # 監視ステータスバナー（全ページ共通）
│   │   └── ThemeToggle.tsx
│   ├── contexts/
│   │   └── LogMonitorContext.tsx      # SSE 接続・status・events・lastSystemMessage
│   └── lib/
│       ├── api.ts                     # fetch ラッパー（snake↔camel 変換）
│       ├── caseConvert.ts             # keysToCamel / keysToSnake
│       ├── logMonitor.ts              # log_monitor_api クライアント
│       ├── suiteCore.ts              # suite_core クライアント
│       └── (他サービスクライアント)
├── index.html
├── package.json
├── tsconfig.json
