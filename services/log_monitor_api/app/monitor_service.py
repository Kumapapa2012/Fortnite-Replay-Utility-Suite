...
class LogMonitorService:
    ...
    def _watch_loop(self) -> None:
        ...
        try:
            ...
            self._push_system_event("scan_error", f"Existing log scan failed: {e}", params={"error": str(e)})
        try:
            self._monitor.watch(poll_interval=0.5)
        except Exception as e:
            self._push_system_event("watch_error", f"Watch loop exited unexpectedly: {e}", params={"error": str(e)})
        finally:
            self._state.status.running = False
    ...
    def _maybe_update_demos_dir(self, replay_dir: str) -> None:
        ...
        try:
            ...
            self._push_system_event("demos_dir_updated", f"demos_dir auto-set: {replay_dir}", params={"dir": replay_dir})
    ...
    async def _post_save_automation(self) -> None:
        ...
        self._push_system_event("post_match_started", "Post-match automation started...")
        ...
        try:
            ...
            self._push_system_event("post_match_refresh", "Refreshing match list...")
            try:
            ...
            if r.status_code != 200:
                self._push_system_event("post_match_fetch_error", f"Failed to fetch match: HTTP {r.status_code}", params={"code": r.status_code})
                return

            matches = r.json().get("matches", [])
            if not matches:
                self._push_system_event("post_match_no_match", "No match found")
                return
            ...
            self._push_system_event("post_match_analyzing", f"Analyzing replay... ({match_id})", params={"matchId": match_id})
                    ...
                    try:
                        ...
                        if 0.0 <= offset < float(vduration):
                            ...
                            if r.status_code == 200:
                                ...
                                self._push_system_event(
                                    "post_match_trim_http_error", f"Trim failed: HTTP {r.status_code}",
                                    params={"code": r.status_code},
                                )
                        else:
                            log.warning("trim offset out of range: %.1f / %.1f", offset, vduration)
                            self._push_system_event(
                                "post_match_trim_offset_error", f"Trim offset out of range: {offset:.1f}s",
                                params={"offset": f"{offset:.1f}"},
                            )
                    except Exception as e:
                        log.warning("trim error: %s", e)
                        self._push_system_event("post_match_trim_error", f"Trim error: {e}", params={"error": str(e)})

            # ── Step 5: Record trim info in sidecar ───────────────────────────
            if trimmed_path is not None and trim_start_sec is not None:
                self._push_system_event("post_match_saving_trim", "Saving trim info...")
                try:
                    async with httpx.AsyncClient(timeout=30.0) as c:
                        await c.patch(
                ...
                self._push_system_event("post_match_mapping_kills", "Mapping kill positions to video...")
                try:
            ...
            self._push_system_event("post_match_error", f"Post-match error: {type(e).__name__}: {e}", params={"error": f"{type(e).__name__}: {e}"})
